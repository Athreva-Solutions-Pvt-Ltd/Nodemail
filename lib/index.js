...
async function main() {
	...
	for(const recipient of recipients) {
		let context = (config.mail.all && config.mail.all.context) ? {...config.mail.all.context, ...recipient} : recipient
		let optionsReplaced = replaceOptionsWithContext(JSON.parse(JSON.stringify(config.mail.each) /* deep copy */), context);
		let mail = {
			...optionsReplaced, context
		};
